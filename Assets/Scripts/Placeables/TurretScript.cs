...
//		this.transform.rotation = Quaternion.Lerp (this.transform.rotation, Quaternion.FromToRotation (Vector3.forward, targetLookDirection.normalized), Time.deltaTime * 10.0f);
		...
		barrel.transform.localRotation = Quaternion.Lerp(barrel.transform.localRotation,Quaternion.RotateTowards (barrel.transform.localRotation, Quaternion.LookRotation (invDir.normalized), 360.0f),Time.deltaTime *10.0f);//Quaternion.Lerp (barrel.transform.localRotation, Quaternion.AngleAxis(Mathf.Rad2Deg*Mathf.Acos(Vector3.Dot (barrel.transform.forward, barrelLookDirection.norma )),barrel.transform.right),Time.deltaTime*100.0f);
